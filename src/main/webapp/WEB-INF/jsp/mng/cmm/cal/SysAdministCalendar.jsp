...
<%
 /**
  * @Class Name  : SysAdministCalendar.jsp
  * @Description : SysAdministCalendar 화면
	...
	if (varParam.sDate) {
		...
		if(sDate.length == 10) {
			if(pForm.init.value != "OK") {
				pForm.init.value  = "OK";
				varForm.action      = "<c:url value='/mng/cmm/cal/SysselectAdministCalendar.do'/>";
...
function fnChangeCalendar(year, month){
	var varForm			= document.all["Form"];
	varForm.action      = "<c:url value='/mng/cmm/SysselectAdministCalendar.do'/>";
...
<form name="Form" action ="<c:url value='/mng/cmm/SysselectAdministCalendar.do'/>" method="post">
