...
<%
 /**
  * @Class Name  : SysRestdeDetail.jsp
  * @Description : SysRestdeDetail 화면
...
function fn_egov_list_Restde(){
	location.href = "<c:url value='/mng/cmm/cal/SysRestdeList.do' />";
}
/* ********************************************************
 * 수정화면으로  바로가기
 ******************************************************** */
function fn_egov_modify_Restde(){
	...
	varForm.action           = "<c:url value='/mng/cmm/cal/SysRestdeModify.do'/>";
...
function fn_egov_delete_Restde(){
	if (confirm("<spring:message code='common.delete.msg'/>")) {
		var varForm				 = document.all["Form"];
		varForm.action           = "<c:url value='/mng/cmm/cal/SysRestdeRemove.do'/>";
		varForm.restdeNo.value   = "${result.restdeNo}";
		varForm.submit();
	}
}
